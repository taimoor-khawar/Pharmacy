<%@page import="com.soft.pharmacy.model.Billline"%>
...
<%
				ArrayList billineslist = null;
				billineslist = (ArrayList)request.getAttribute("billineslist");
				
				Bill bill = new Bill();
				bill = (Bill)request.getAttribute("Bill");
						...
						<div class="panel-body">
							<div class="row">
								...
								<form name="newOrder" id="newOrder" role="form"
									method="post" action="/POS/order/updateorder" style="padding: 20px 20px">
									<input type="hidden" name="EnterprisedID" id="EnterprisedID" value="<%=uss.getSysuser().getsysUserID()%>">
									<input type="hidden" name="CustomerID" id="CustomerID" value="<%=bill.getCustomerID()%>">
									<input type="hidden" name="BillID" id="BillID" value="<%=bill.getBillID()%>" />
									<input type="hidden" name="BillName" id="BillName" value="<%=bill.getBillName()%>" />
									<div class="col-lg-6">
										<label>Customer</label> <label><%=bill.getCustomerName() %></label>
							...
							<table width="100%" 
								...
								<tbody id="ordertable">
									<%for(int i=0;i<billineslist.size();i++){
										Billline billline = (Billline)billineslist.get(i);%>
										<tr>
				 							<input type="hidden" name="ProductCartID" id="ProductCartID" value="<%=billline.getProductID()%>" />
				 							<input type="hidden" name="BilllineID<%=i %>" id="BilllineID<%=i %>" value="<%=billline.getBilllineID()%>" />
				 							<td><%=billline.getProductName() %></td>
				 							<td><input type="text" style=width:120px; name="ProductPrice<%=billline.getProductID()%>" onkeyup="calculateRow();" value="<%=billline.getProductAmount() %>" class="ProductPrice form-control" onkeypress="onlynumbers(this)"/></td>
				 							<td><input style=width:120px; class="ProductQuantity form-control" type="text" name="ProductQuantity<%=billline.getProductID()%>" value="<%=billline.getProductQuantity() %>" onkeyup="calculateRow()" onkeypress="onlynumbers(this)"/></td>
				 							<td><input style=width:120px; class="NetPrice form-control" type="text" name="NetPrice<%=billline.getProductID()%>" onkeypress="onlynumbers(this)" value="<%=billline.getBilllineAmount() %>"/></td>
				 							<td><input style=width:120px; class="Discount form-control" type="text" name="Discount<%=billline.getProductID()%>" onkeyup="calculateRow()" value="0" onkeypress="onlynumbers(this)" value="<%=billline.getBilllineDiscount() %>"/></td>
				 							<td><input style=width:120px; class="TotalPrice form-control" type="text" name="TotalPrice<%=billline.getProductID()%>" onkeypress="onlynumbers(this)" value="<%=billline.getBilllineDueAmount() %>"/></td>
			 							</tr>
										<%} %>
								</tbody>
							</table>
							...
							<div class="col-lg-6" style="margin-top: 10px;"></div>
									<div class="col-lg-6" style="margin-top: 10px;width: 300px;margin-left: 210px;">
										<label>Order Total</label> <input class="form-control" type="text"
											...
											placeholder="OrderTotal" value="<%=bill.getBillDueAmount()%>">
									</div>
							<div class="col-lg-6" style="margin-top: 10px;"></div>
